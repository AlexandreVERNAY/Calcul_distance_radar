% Commandes de remise à zéro de Matlab
...
Donnees_emission  = load('z_SignalRadarEMIS.txt'); % Chargments des valeurs du signal émis
Donnees_reception = load('z_SignalEchoRadar2.txt');% Chargments des valeurs du signal reçu
...
figure(13);
hold on
plot(Temps, Signal_emis,'Color','#D95319'); % Singal émis en temporelle
plot(Temps, Signal_recu,'Color','#7E2F8E'); % Singal reçu en temporelle
...
% Vecteur Fréquence Freq
Freq = PasFreq*((-(NbPoint-1)/2):((NbPoint-1)/2));

% Temps echantillonage
Te = Temps(2) - Temps(1); % Temps entre deux échantillon

%% 4 - Transformés de Fourier
% Signal émis
% TF du signal émis
...
% Autocorrelation du signal émis
AutoCorr_Signal_emis = xcorr(Signal_emis);

% Intercorrelation du signal signal reçu et du signal émis
